...
class AccountSummaryViewController: UIViewController {
    ...
    // Networking
    var profileManager:  ProfileManageabal = ProfileManager()
...
extension AccountSummaryViewController {
    ...
    private func configureTableCells(with accounts: [Account]) {
        accountCellModels = accounts.map {
            AccountSummaryModel(accounType: $0.type,
                                accountName: $0.name,
                                balance: $0.amount)
        }
    }
    ...
    private func displayError(_ error: NetworkError) {
        let titleAndMessages = titleAndMessages(for: error)
        self.showErrorAlert(title: titleAndMessages.0, message: titleAndMessages.1)
    }
    
    private func titleAndMessages(for error: NetworkError) -> (String, String) {
        let title: String
        let message: String
        switch error {
        case .serverError:
...
extension AccountSummaryViewController {
    func titleAndMessageForTesting(for error: NetworkError) -> (String, String) {
        return titleAndMessages(for: error)
    }
}
